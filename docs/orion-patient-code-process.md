...
La secuencia no tiene un límite predeterminado. Empieza en `01` y continúa de forma correlativa (`02`, `03`, etc.). Cada centro mantiene su propia secuencia.

Los códigos nuevos siguen el formato de dos bloques numéricos definido arriba.

La generación se realiza en el servidor y queda registrada antes de mostrar el código en el formulario. La plataforma aplica restricciones de unicidad sobre el código completo y sobre la combinación de centro y secuencia. Por ello, dos altas simultáneas no pueden producir el mismo código. Puede haber saltos de numeración si se inicia una selección que finalmente no se completa; estos saltos son intencionados y preservan la garantía de no reutilización.
...
La seqüència no té un límit predeterminat. Comença a `01` i continua de forma correlativa (`02`, `03`, etc.). Cada centre manté la seva pròpia seqüència.
